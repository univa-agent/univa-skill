...
class StorageService {
  ...
  async saveMediaItem(projectId: string, mediaItem: MediaItem): Promise<void> {
    ...
    const metadata: MediaFileData = {
      ...
      serverPath: mediaItem.serverPath,
    };
  ...
  async updateProjectMediaServerPathPrefix(
    projectId: string,
    oldPrefix: string,
    newPrefix: string
  ): Promise<void> {
    ...
    await Promise.all(
      mediaIds.map(async (id) => {
        ...
        if (!metadata?.serverPath?.startsWith(oldPrefix)) return;
        await mediaMetadataAdapter.set(id, {
          ...metadata,
          serverPath: metadata.serverPath.replace(oldPrefix, newPrefix),
        });
      })
    );
  }
